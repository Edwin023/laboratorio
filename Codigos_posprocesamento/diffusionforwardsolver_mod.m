function phi = diffusionforwardsolver_mod(x,n,mua1,mus1,go1,tau,lambda,rho,z,w,ell,mua2,mus2,go2,cutoff)
...
if mua2 == 0
    ...
    phi = 1/(4*pi*D1) * (exp(-alpha1 * r1)./r1 - exp(-alpha1*rb)./rb);
    ...
    alpha1sq = (D1*S.^2 + mua1 + 2*mus1*k0^2*tau*x(1) + 1i*w/v) / D1;
    alpha1 = sqrt(alpha1sq);
    alpha2sq = (D2*S.^2 + mua2 + 2*mus2*k0^2*tau*x(2) + 1i*w/v) / D2;
    alpha2 = sqrt(alpha2sq);
        
    %Fourier transform of phi taken from solution by Kienle et al, Applied
    %Optics, V 37, pages 779-791, 1998.
    
         phit = (sinh(alpha1*(zb + z0)) ./ (D1*alpha1)) .* ((D1*alpha1.*cosh(alpha1*(ell-z)) + D2*alpha2.*sinh(alpha1*(ell-z))) ...
         ./ (D1*alpha1.*cosh(alpha1*(ell+zb)) + D2*alpha2.*sinh(alpha1*(ell+zb)))) - sinh(alpha1*(z0-z))./(D1*alpha1);
