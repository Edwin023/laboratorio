...
elseif isstruct(varargin{1}) % Options=LMFsolve(Options,'Name','Value',...)
    ...
    for i=2:2:nargin-1
        ...
        elseif strncmp(name,'s',1), xf.ScaleD   = value;
         elseif strncmp(name,'n',1), xf.Nam   = value; 
...
elseif ischar(varargin{1})   % check for Options=LMFSOLVE('Name',Value,...)
   Pnames=char('display','funtol','xtol','maxiter','scaled','nam');
...
filename1=options.Nam;
x  = xc(:);
lx = length(x);
  filename=[filename1 num2str(1)];
   save('optimizacion.mat','filename');
...
%   filename=['optimization_Jac' num2str(1)];
%    save('optimizacion.mat','filename');
J = finjac(FUN,r,x,epsx,1,filename1);
%~~~~~~~~~~~~~~~~~~~~~~~
nfJ = 2;
A = J.'*J;                    % System matrix
v = J.'*r;
...
cnt2=2;
t_interacciones=zeros(maxit,5);
Jac=zeros(length(r),length(x),cnt2);
r_vec=zeros(maxit,length(r));
while cnt<maxit && ...          %   MAIN ITERATION CYCLE
    ...
    %xd = x-d;
    xd = x+d;
   filename=[filename1 num2str(cnt2)];
   save('optimizacion.mat','filename');
    rd = feval(FUN,xd);
%   ~~~~~~~~~~~~~~~~~~~
    nfJ = nfJ+1;
   ...
   t_interacciones(cnt,1)=cnt;
   t_interacciones(cnt,2)=S;
   t_interacciones(cnt,3)=x;
   t_interacciones(cnt,4)=d;
  % t_interacciones(cnt,5)=r;
  r_vec(cnt,:)=r';
   Jac(:,:,cnt)=J;
    if Sd<S
        S = Sd; 
        x = xd; 
        r = rd;
%    filename=['optimization_Jac' num2str(cnt2)];
%    save('optimizacion.mat','filename');
        J = finjac(FUN,r,x,epsx,cnt2,filename1);
%       ~~~~~~~~~~~~~~~~~~~~~~~~~
        nfJ = nfJ+1;
        A = J'*J;       
        v = J'*r;
...
save([filename1 '.mat'],'t_interacciones','Jac','r_vec')
xf = x;                         %   final solution
if cnt==maxit
    cnt = -cnt; 
end                             %   maxit reached
...
%   %%%%%%
function J = finjac(FUN,r,x,epsx,cnt2,filename)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
lx=length(x);
J=zeros(length(r),lx);
for k=1:lx
    ...
    xd=x;
    xd(k)=xd(k)+dx;
    filename=[filename '_Jac' num2str(cnt2) '_int' num2str(k)];
     save('optimizacion.mat','filename');
    rd=feval(FUN,xd);
%   ~~~~~~~~~~~~~~~~    
    J(:,k)=((rd-r)/dx);
