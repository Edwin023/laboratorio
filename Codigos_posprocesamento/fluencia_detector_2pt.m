function phi_2pt=fluencia_detector_2pt(sess_name,ndet,nx,ny,det,dr,h)  


file_stats = dir([sess_name num2str(h) '_R.bin']);
    data_size = file_stats.bytes/(nx*ny);
fid = fopen([sess_name num2str(h) '_R.bin'], 'rb');
    if(data_size==4)
    ...
    fclose(fid);
    data_2pt = reshape(data_2pt, [nx,ny]);
