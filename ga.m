...
while (gaDat.gen<gaDat.MAXGEN),
    gaDat.gen=gen;
     mkdir(['arquivos_his/generacion' num2str(gen+1) ]);
     mkdir(['arquivos_F/generacion' num2str(gen+1) ])
     mkdir(['arquivos_T/generacion' num2str(gen+1) ])
     mkdir(['arquivos_mci/generacion' num2str(gen+1) ])
    gaDat=gaevolucion(gaDat,gen+1);  
...
%% ----------------------------------------------------
function gaDat=gaevolucion(gaDat,nint)
% One generation -------
Chrom=gaDat.Chrom;
nind=size(Chrom,1);
ObjV=inf(nind,1);
